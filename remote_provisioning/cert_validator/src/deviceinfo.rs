...
pub fn check(deviceinfo: &[Value]) -> Result<()> {
    ...
    let version = verified_info
        .map_lookup(String::from("version"))?
        .as_i64()?;
    ...
    match version {
        ...
        2 => {
            verified_info
                .map_lookup(String::from("brand"))?
                .as_string()?;
            verified_info
                .map_lookup(String::from("manufacturer"))?
                .as_string()?;
            verified_info
                .map_lookup(String::from("product"))?
                .as_string()?;
            verified_info
                .map_lookup(String::from("model"))?
                .as_string()?;
            verified_info
                .map_lookup(String::from("device"))?
                .as_string()?;
            ...
            verified_info
                .map_lookup(String::from("system_patch_level"))?
                .as_date()?;
            verified_info
                .map_lookup(String::from("boot_patch_level"))?
                .as_date()?;
            verified_info
                .map_lookup(String::from("vendor_patch_level"))?
                .as_date()?;
    ...
    let security_level = verified_info
        .map_lookup(String::from("security_level"))?
        .as_string()?;
